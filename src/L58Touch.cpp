...
{
    uint16_t x, y, s;
    if (digitalRead(CONFIG_TOUCH_INT))
    {
        if (scanPoint())
        {
            getPoint(x, y, s, 0);
...
void L58Touch::readBytes(uint8_t *data, uint8_t nbytes)
{
...
void L58Touch::getPoint(uint16_t &x, uint16_t &y, uint16_t &s, uint8_t index)
{
    if (index >= 4)
        return;
    x = data[index].x;
